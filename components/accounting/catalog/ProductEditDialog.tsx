...
export function ProductEditDialog({
  ...
  // Load custom field defs on mount
  useEffect(() => {
  ...
  useEffect(() => {
    if (!open) return;
    setActiveTab("basic"); // Reset to basic tab when dialog opens
    ...
    if (editingProduct) {
      ...
      setNewDiscountValue("");
      setNewDiscountValidTo("");
    }
  }, [open, editingProduct, units]);
  ...
  const handleAcceptSuggestion = async (suggestion: VariantSuggestion) => {
    ...
    try {
      const res = await fetch(`/api/accounting/products/${editingProduct.id}/variant-links`, {
        ...
        body: JSON.stringify({
          linkedProductId: suggestion.productId,
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <Tabs defaultValue="basic" className="w-full" onValueChange={(value) => setActiveTab(value)}>
            ...
            <div className="grid gap-4 py-4">
              ...
              {showNewCfForm ? (
                ...
                <div className="flex items-center gap-3 pt-1">
                  ...
                  <Link href="/references?tab=customfields" onClick={() => onOpenChange(false)}>
                          ...
                          <div className="flex items-center gap-2 min-w-0">
                            ...
                            <div className="min-w-0">
                              ...
                              <div className="text-xs text-muted-foreground">
                                ...
                                {vl.linkedProduct.salePrice != null && (
                                  <span className="ml-2">{formatRub(vl.linkedProduct.salePrice)}</span>
                                )}
                              </div>
                            </div>
                          </div>
          ...
          {/* === Suggestions Tab (edit only) === */}
          {editingProduct && (
            <TabsContent value="suggestions">
              <div className="grid gap-4 py-4">
                <p className="text-xs text-muted-foreground">
                  Умные подсказки находят похожие товары по артикулу, названию и характеристикам. Вы можете принять или отклонить каждую подсказку.
                </p>
                ...
                {!suggestionsLoading && suggestions.filter((s) => !dismissedSuggestions.has(s.productId)).length === 0 && (
                  <p className="text-sm text-muted-foreground">Подсказок не найдено. Попробуйте добавить товары с похожими артикулами или названиями.</p>
                )}
                        ...
                        <div key={s.productId} className="flex items-center justify-between px-3 py-2 text-sm">
                          <div className="flex items-center gap-2 min-w-0 flex-1">
                            ...
                            <div className="min-w-0 flex-1">
                              <div className="flex items-center gap-2 flex-wrap">
                                ...
                                <Badge
                                  variant={s.confidence >= 80 ? "default" : s.confidence >= 60 ? "secondary" : "outline"}
                              ...
                              <div className="text-xs text-muted-foreground">
                                ...
                                {s.product.salePrice != null && (
                                  <span className="ml-2">{formatRub(s.product.salePrice)}</span>
                                )}
                          ...
                          <div className="flex items-center gap-1 shrink-0">
                            ...
                            <Button
                              ...
                              size="icon"
                              className="h-7 w-7"
                              onClick={() => handleDismissSuggestion(s.productId)}
                            >
                              <X className="h-3.5 w-3.5" />
                            </Button>
                          </div>
                ...
                {productDiscounts.length > 0 && (
                  <div className="space-y-2">
                        ...
                        return (
                          <div key={d.id} className="flex items-center justify-between px-3 py-2 text-sm">
                            <div>
                              ...
                              {dp != null && (
                                <span className="text-green-600 ml-2">{formatRub(dp)}</span>
                              )}
                              {d.validTo && (
                    ...
                    {calcDiscountedPrice() != null && (
                      <p className="text-sm">
                        Итого со скидкой: <span className="text-green-600 font-medium">{formatRub(calcDiscountedPrice()!)}</span>
                      </p>
                    )}
                  </div>
                )}
